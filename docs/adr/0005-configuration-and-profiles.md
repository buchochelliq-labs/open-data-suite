...
- **Flatten every layer to `key path → value`, overwrite in precedence order (chosen).**
  ...
  leaves, so they are replaced, never merged element by element.
...
|---|---|---|
| user | `$XDG_CONFIG_HOME/ods/config.toml`, else `$HOME/.config/ods/config.toml`, else `%APPDATA%\ods\config.toml` | no |
...
A missing file is not an error. `explain` lists which files were found.
...
6. environment variables `ODS__<SECTION>__<KEY>`, e.g. `ODS__OUTPUT__WIDTH=120`. The
   value is parsed as a TOML value when possible (`120`, `true`, `"x"`), otherwise as a
   string. Variables are applied in sorted order, so the result does not depend on
   environment order.
...
- Under `providers.*`, any key named like a credential (`token`, `password`, `secret`,
  `api_key`, `private_key`, `credentials`, `client_secret`, or a name ending in
  `_<word>`, such as `access_token`) **must** be a reference. A plaintext value is an
  error (ODS-E0103), whichever layer set it. The error message never includes the value.
